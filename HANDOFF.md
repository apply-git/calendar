...
## 建議新增功能（2026-07-21 規劃，尚未實作）

依目前完整度，下一階段優先強化「資料可靠性、整合、智慧排程」，而非只增加按鈕。以下為建議清單，依價值與依賴排序。

### 建議實作順序（總覽）

1. 自動化測試與資料檢查工具（先建安全網）
2. 重複行程單次編輯例外（僅這次／這次及之後／整個系列）
3. 行程衝突與空檔偵測
4. 通知互動（完成／延後／再提醒）
5. 逐筆雲端同步與衝突解決（取代整包覆蓋）
6. Google Calendar 整合或家庭共享日曆

> 最建議「先做」：**重複行程單次編輯 + 行程衝突偵測**，使用價值高，且不必立即大改雲端架構。
...
| 2 | 編輯單次重複行程 | 支援「僅這次／這次之後／整個系列」 | 中高 |
| 3 | 行程衝突與空檔偵測 | 自動提示時間重疊、尋找可用時段 | 中 |
...
#### 2. 完整重複行程例外
編輯重複行程時提供三選項：僅這一次／這次及之後／整個系列。資料可增加 `recurrenceId` 與「單日覆寫」機制，補足目前 `excludedDates` 只能「排除」不能「單獨修改」的限制。
...
### 品質面補強
- 自動化測試：至少涵蓋 `occursOnDate()`、自然語言解析、備份還原、同步合併。
