...
- **每張圖都有紅色標註**：紅色虛線圈選＋紅色引線＋紅色粗體標號文字，figcaption 呼應圖上①②③編號。新增 CSS 變數 `--anno`（淺 `#e5484d`／深 `#ff6b6b`）與 `--anno-bg`（文字底色描邊），淺色/深色/prefers-color-scheme 三處都定義
- 歡迎卡片與教學頁的**表頭在手機顯示「手機行程表」**：沿用既有 `displayAppTitle()`（app-08-data.js）同一套規格，教學頁則依「選到哪一軌」切換而非視窗寬度
- 歡迎卡片手機版改**懸浮置中**（原本跟其他 dialog 一樣貼底）；此覆寫與 `dialog.task-dialog` 特異性相同，**必須排在它之後**才會生效——踩坑寫入 NOTES 第 19 條（連帶記錄 PWA 內部導航用 `location.href` 的理由）
- `CACHE_NAME` v47→v49
- **做法說明（2026-08-09 使用者已定案：擬真即可，不需真截圖）**：18 張圖是**擬真繪製的 inline SVG**，不是螢幕截圖。好處是零外連、離線可用、可縮放、不會因改版過期。**這是最終決定，之後不要再提議換成真截圖**；新增章節時照同樣方式自繪（桌機套瀏覽器視窗外框、手機套手機外框，配紅圈＋紅色引線＋紅字標註）
- **剩什麼**：教學頁是整頁導航，iOS standalone 沒有返回手勢，目前只靠頁首/頁尾兩顆「回行事曆」；使用者若嫌不順，可加固定浮動返回鈕或改成 App 內全螢幕覆蓋層（尚未決定）
